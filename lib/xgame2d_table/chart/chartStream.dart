...
import 'package:tournament_client/xgame2d_table/chart/chartScreen.dart';

class ChartStreamPage extends StatefulWidget {
  const ChartStreamPage({Key? key}) : super(key: key);
  @override
  State<ChartStreamPage> createState() => _ChartStreamPageState();
}

class _ChartStreamPageState extends State<ChartStreamPage> {
  @override
  ...
  void initState() {
    ...
    _throttledStream = mySocket.dataStream
        .debounceTime(const Duration(milliseconds: 500)) // Throttle updates every 500ms
      ...
      builder: (context, state) {
        return StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            // debugPrint('X: $positionXCombined');
            // debugPrint('Y: $positionYCombined');
              ...
              final bool hasDataChanged = !listEquals(state.valueDisplay, valueDislay);
 ...
 void  arrangeData(List<Map<String, dynamic>> dataList) async{
  ...
  // valueDislay =  nextChips;
  // valueDisplayPrev =  initialChips;
...
void arrangeDataSkip(List<dynamic> dataList, List<int> skipList) {
  ...
  // Debugging Output
  // debugPrint('arrangeDataSkip:inputNumber: $inputNumber Members Sorted: $members initialChips Sorted : $initialChips nextChips Sorted: $nextChips valueDisplay $valueDislay  valueDisplayPrev: $valueDisplayPrev');
}
}
